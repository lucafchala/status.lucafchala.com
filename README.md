...
| `fotos.lucafchala.com` | gallery renders · **`/api/healthz`** parsed (KV alive, `events` count, D1 consent log, PBKDF2 `hashMs` within budget) · `/dashboard` serves the login form · `/manifest.json` is valid PWA JSON · `/termos` (LGPD) and `/suporte` render |
...
The dashboard shows each service's checks in an expandable panel: a healthy service collapses behind a `N verificações ok` toggle, while any service with a failing check **auto‑expands** so the problem is never hidden. fotos gets the deepest coverage because its `/api/healthz` exposes `{ ok, kv, events, d1, hashMs }`.
