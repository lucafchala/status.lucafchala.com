...
- **State (KV):** `subscribers` `[{ email, token, subscribedAt }]`, `last_status` `{name: status}`, `history` `[{ name, from, to, at, severity, problems }]` (newest‑first, 48 h window, capped at 60 entries), `notify_sent:{name}` cooldown markers (TTL 1 h).
