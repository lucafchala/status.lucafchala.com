...
| `fotos.lucafchala.com` | **deliberately exhaustive (14 checks)** — gallery renders · **`/api/healthz`** dissected field‑by‑field (KV alive + **latency budget**, `events` count, D1 consent log, PBKDF2 `hashMs`, **daily‑cron heartbeat staleness**, and the `resend`/`turnstile` hardening secrets) · **security headers** present on `/termos` (CSP, HSTS, nosniff, frame, referrer, permissions) · `/dashboard` serves the login form · `/manifest.json` is a complete PWA manifest (name, icons, `start_url`, `theme_color`) · `/icon.svg` + `/og-coming-soon.png` serve with the right type · `/sitemap.xml` is valid XML (`<urlset>`) · `/robots.txt` advertises the sitemap · `/.well-known/security.txt` is RFC 9116‑valid and **not expired** · `/.well-known/gpc.json` is `gpc:true` · `/termos` (LGPD), `/privacidade`, `/suporte` render · a **negative routing probe** confirms an unknown path still 404s |
...
The dashboard shows each service's checks in an expandable panel: a healthy service collapses behind a `N verificações ok` toggle, while any service with a failing check **auto‑expands** so the problem is never hidden. fotos gets **intentionally overkill** coverage: 14 functional checks where the service status is the *worst* of all of them, and every failing check is named individually (both in the panel and in the alert email, which now lists **all** of a service's problems, not just the first). Its `/api/healthz` exposes a deep payload — `{ ok, kv, events, d1, hashMs, kvLatencyMs, d1LatencyMs, cron, config, colo, … }` — and the status sweep mines every field, so any anomaly (a stalled cron, a slow KV read, a dropped security header, an expiring security.txt, a missing hardening secret) surfaces here before it becomes a visible outage. The healthz payload is **KV-frugal** (two reads: `events` + the cron heartbeat) despite its depth, and the security-header check targets the static `/termos` page so it adds no KV read on fotos' side.
