...
```
            ...
            │  subscribe form:       POST /api/subscribe  → confirmation e-mail → GET /api/confirm shows,
            │                                                          POST confirms (double opt-in)
            │  unsub link → GET /api/unsubscribe shows, POST performs — RFC 8058 one-click
            └───────────────────────────────────┬───────────────────────────────────────┘
                  ...
                  & provider status APIs                              notify_sent:{name}:{to}, pending_sub:*
```

- **Static front end:** `index.html` (markup only) + `app.css` + `app.js` + `tema.js` (the theme, applied synchronously in `<head>` so the page doesn't flash) render the dashboard. **Theme and language are shared with the whole ecosystem** through the `lf_theme` / `lf_lang` cookies on `.lucafchala.com` (same code as paste/keys; `localStorage` as fallback, then the OS/browser). The page chrome is **PT/EN** (`S` in `app.js`, `data-i18n` in the markup; the Portuguese in the HTML is the no-JS text). Data from the server — service names, check labels and details, quota rows — stays Portuguese: it is the same text the alert e-mails and the history carry. Every `<script>` has `data-cfasync="false"` (Cloudflare Rocket Loader). **No inline script, no inline style, no `on…` attribute, no third-party resource** — clicks go through one delegated listener keyed by `data-action` — so the CSP in `_headers` is `script-src 'self'; style-src 'self'; font-src 'self'` with no `'unsafe-inline'`. `tests/pagina.test.mjs` pins the files to that; a real browser is the actual check (an inline handler brought back shows up as "Refused to execute inline event handler" and a dead button).
...
- **Change detection runs server-side**: each fresh `/api/status` sweep is compared against `last_status` in KV; transitions email the admin + subscribers (Resend batch). Nothing a client sends can trigger or shape an email. The rules:
  - **Cooldown per service *and* target state** (`notify_sent:{name}:{to}`, 1 h): the same "down" twice in an hour is noise, but the recovery — or an escalation from degraded to down — is news and always goes through.
  - **Batches of ≤ 100** (Resend's limit), owner first, and every response is checked. The cooldown is written **only after a successful send**; a refused send goes to `alert_pending` (TTL 6 h, only written on failure) and the next sweep retries whatever is still true, up to 3 attempts. Before, a refused batch was silent and burned the cooldown.
  - **First sighting counts**: a service that appears already broken counts as leaving `up` (except on the very first sweep of an empty `last_status`, so a new install doesn't burst).
  - **Quota and TLS rows are owner-only** — they go to `NOTIFY_TO`, never to subscribers. If `/api/quota-stats` can't be read, the previous quota rows are carried forward instead of vanishing (and re-alerting when it comes back).
  - A sub-check that throws becomes a named `degraded` row ("erro interno na verificação") instead of a 500; a network error gets one quick retry before it counts as `down`.
...
- **State (KV):** `subscribers` `[{ email, token, subscribedAt, confirmedAt? }]`, `pending_sub:{sha256(email)}` (unconfirmed sign-ups, TTL 24 h), `alert_pending` (alerts waiting for a retry), `last_status` `{name: status}` (services **and** `cota · …` / `TLS · …` rows, so quota crossings alert through the same path), `history` `[{ name, from, to, at, severity, problems }]` (newest‑first, 48 h window, capped at 60 entries), `notify_sent:{name}:{to}` cooldown markers (TTL 1 h).
...
- **Free-tier headroom:** `/api/quota-stats` reads the Cloudflare GraphQL Analytics API for KV ops/storage, Workers requests/CPU and D1 rows, plus certificate expiry from the zone's certificate packs. It needs a read‑only API token; without one the panel says **"não monitorado"** with the reason rather than showing zeros or disappearing, since "unmonitored" must not look like "fine".

### Endpoints

| Route | Method | Behavior | Needs |
|---|---|---|---|
| `/api/status` | GET | **Functional** health‑checks of the 13 first‑party services in parallel (GET, 10 s timeout). Each service has a primary availability probe (status code + latency + a content marker proving the right page rendered) plus optional sub‑checks — running server‑side means it can read response bodies cross‑origin, which the browser can't. Returns `{ services: [{ name, url, status, statusCode, rt, checks, problems }], checkedAt }`, where `checks` is `[{ label, status, detail }]` and `problems` is a list of human‑readable failures. A service's status is the **worst** of all its checks. **Base rules:** HTTP ≥ 500 → `down`; 400–499, slow (`rt` > 2500 ms), unexpected/empty content, missing data files, or a failing `/api/healthz` → `degraded`/`down`. Edge-cached 30 s; sends `Access-Control-Allow-Origin: *`, kept only for dash builds older than dash#34; other sites read `/api/resumo` (the homepage since #52, the dash since dash#34), never this endpoint, since a request here can sweep. On fresh sweeps, also runs server-side change detection + alert emails (which list **every** failing check for a changed service, not just the first). | (emails need `RESEND_API_KEY`, `NOTIFY_TO`, `STATUS_KV`) |
| `/api/third-party-status` | GET | Checks provider status pages (GitHub, Cloudflare, Anthropic, Resend, Google Drive via the Workspace dashboard feed), 8 s timeout, with provider‑specific parsing (Cloudflare filtered to Brazil PoPs; Statuspage `maintenance` → degraded; Google: only incidents **without `end`** on the exact product). A status page that doesn't answer, answers 403/404 or answers something unparseable is **`unknown` ("sem dados")** — never green, never red. Google Fonts was dropped (the ecosystem self-hosts its fonts). Edge-cached 2 min under a fixed key. | — |
...
| `/api/subscribe` | POST / GET | **Double opt-in.** POST stores a *pending* sign-up (`pending_sub:{hash}`, TTL 24 h) and e-mails a confirmation link; nobody is added to the list until they click it. One link per mailbox per 24 h: the pending key is the hash of the address with any `+tag` removed (and, for Gmail, the dots), so `ana+1@gmail.com` and `a.na+2@googlemail.com` share it — repeated POSTs can't mail-bomb anyone; the link still goes to the address typed. Returns `{ ok, pending }` — the same answer whether the address is new, pending or already subscribed, so the endpoint doesn't reveal who is on the list. Optional **Turnstile**: when `TURNSTILE_SECRET_KEY` is set, a valid token is required (GET returns `{ turnstile: siteKey }` so the page loads the widget). **The only public endpoint that writes KV *and* makes a third party send mail to an address the caller chooses** — so it is gated: same‑site check (`Sec-Fetch-Site`, inforjável por script), a per‑IP throttle held in isolate memory (IPv6 counted per /64), a hard cap of 2000 subscribers and, with `STATUS_DB`, a global cap of 50 confirmation e‑mails per day (a D1 counter, so a refused attempt costs no KV write). Errors never name a missing binding and never echo Resend's raw response body; both go to the log instead. | `RESEND_API_KEY`, `NOTIFY_TO`, `NOTIFY_FROM`, `STATUS_KV` |
| `/api/confirm?id=…&token=…` | GET / POST | Second half of the opt-in. **GET only shows a "Confirmar inscrição" button** (mail scanners open every link; if GET confirmed, the scanner would confirm for someone who never read the message); **POST** adds the address to `subscribers`, deletes the pending entry and sends the welcome e-mail with the unsubscribe link. The link's token becomes the unsubscribe token. Same strict CSP as the unsubscribe page. | `STATUS_KV` (`RESEND_API_KEY` for the welcome mail) |
...
|---|---|
| `lucafchala.com` | homepage renders (content marker) · **negative routing probe** (`/__status_probe_404__` must 404 — `_redirects` is generated by dash and applies before files, so a catch-all would turn every broken link into a 200) |
...
| `url.lucafchala.com` | app renders · `/data.json` is valid JSON with a `redirects` array · **data freshness** · **negative routing probe** |
...
| `status.lucafchala.com` *(self)* | dashboard renders (marker `monitoramento de serviços`, in the static footer — CI checks it's there) · own **`/api/healthz`** answers · **alert configuration** read from its own environment (same project) — `STATUS_KV` / `RESEND_API_KEY` / `NOTIFY_TO` missing shows as "configuração incompleta" without naming which · **Resend delivery** verified against the live API (key still accepted, sender domain still verified, latency within budget). A *total* outage can't self‑report — the GitHub Actions monitor's non‑200 is the backstop. |
...
**The page** follows the layout of industry status pages: a **summary band** with the state of everything in one sentence ("Todos os sistemas operacionais", or *who* is wrong and since when — "Treino degradado há 12 min"); each service with its state as **icon + word** (never color alone), **history bars** — 90 daily bars with `STATUS_DB` (30 on a phone), 48 hourly bars rebuilt from the transition log without it (24 on a phone) — with the window's availability, a per-bar detail on hover/tap, 24 h/48 h uptime, and a **deploy badge** when the service declares a recent version; **recent incidents** as a timeline grouped by day (onset → recovery, duration, first failing check, ongoing ones first); response-time sparklines with **deploys marked** as dashed lines; third parties; and Cloudflare quotas. Accessibility: landmarks and headings, a skip link, an `aria-live` region that announces **changes** of state (not every refresh), `<time datetime>` on every date, visible focus, `prefers-reduced-motion` honoured, and state colors measured for **WCAG AA in both themes** (`tests/pagina.test.mjs` reads the tokens from `app.css` and fails below 4.5:1 — the old `--down` was 2.6:1 on dark). axe-core reports 0 violations at 360 px and desktop, light and dark (the previous page had 3, including 94–104 contrast failures).
...
|---|---|---|---|
| `RESEND_API_KEY` | secret | subscribe, notify | Resend API key used to send confirmation, welcome + alert emails |
...
| `STATUS_ADMIN_TOKEN` | secret | *(optional)* healthz detail | Any long random string. `curl -H "X-Status-Token: …" https://status.lucafchala.com/api/healthz` returns the config booleans and counts; without it the endpoint only says `ok`. |
| `TURNSTILE_SITE_KEY` / `TURNSTILE_SECRET_KEY` | var / secret | *(optional)* subscribe | Turnstile on the sign-up form. **Also requires** adding `https://challenges.cloudflare.com` to `script-src` and `frame-src` in `_headers` (and updating the "sem origem de terceiro" assertion in `tests/pagina.test.mjs`) — the default CSP deliberately allows no third-party origin, and double opt-in + per-IP throttle + one link per address per day already bound abuse. |

If `STATUS_KV` is missing during a subscribe, the function answers 503 and emails the admin; if the confirmation e-mail can't be sent, the pending entry is deleted and the caller gets a 502 (so they can try again). Existing subscribers from before the double opt-in stay subscribed.
...
**Quota crossings are alerted, not just coloured.** Each quota (and each zone's certificate) is tracked through the *same* pipeline as a service: it enters `last_status`, so crossing 75 % emails at `ATENÇÃO`, crossing 95 % escalates to `CRÍTICO`, and both inherit the hourly cooldown, the batched email and the transition log — but go **only to the owner** (`NOTIFY_TO`). Since the scheduler sweeps `/api/status` every 10 minutes, this runs whether or not anyone has the dashboard open — which is the whole point, given the panel is otherwise only true while someone is looking at it.
...
└── functions/
    └── api/
        ...
        ├── third-party-status.js    # GET  /api/third-party-status  — checks GitHub/Cloudflare/Anthropic/Resend/Google Drive
        ├── subscribe.js             # POST /api/subscribe           — pending sign-up + confirmation mail (same-site + per-IP throttle + cap + optional Turnstile)
        ├── confirm.js               # GET  shows the confirm button, POST adds to the list + welcome mail
...
Uses the shared ecosystem design system — dark `#0d0c0a` / amber `#c08030`, **Cormorant Garamond** + **JetBrains Mono** (self-hosted, see above), light/dark toggle — **extended** with status‑state colors chosen for WCAG AA against the background in each theme: dark `--up #5fb878`, `--degraded #d4a03c`, `--down #e06666`; light `--up #23703d`, `--degraded #8a5a0a`, `--down #a8282a`. Status labels are **operacional** / **degradado** / **fora do ar** / **sem dados** (EN: operational / degraded / down / no data), always with an icon (✓ ! ✕ ?). The alert emails reuse the same tokens inline.
...
- [x] subscribe hardening — double opt-in, uniform answers, optional Turnstile (2026-09)
- [x] status dots on the `dash` page — `/api/status` now sends `Access-Control-Allow-Origin: *`
- [x] latency trending — done (`/api/latency-trends`, D1 series)
- [ ] homepage status dots (lucafchala/lucafchala.com#21)
