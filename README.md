...
| `fotos.lucafchala.com` | **deliberately exhaustive (14 checks)** — gallery renders · **`/api/healthz`** dissected field‑by‑field (KV alive + **latency budget**, `events` count, D1 consent log, PBKDF2 `hashMs`, **daily‑cron heartbeat staleness**, `adminConfigured`, and the `resend`/`turnstile` hardening secrets) · **security headers** present on `/` (CSP, HSTS, nosniff, frame, referrer, permissions) · `/dashboard` serves the login form · `/manifest.json` is a complete PWA manifest (name, icons, `start_url`, `theme_color`) · `/icon.svg` + `/og-coming-soon.png` serve with the right type · `/sitemap.xml` is valid XML (`<urlset>`) · `/robots.txt` advertises the sitemap · `/.well-known/security.txt` is RFC 9116‑valid and **not expired** · `/.well-known/gpc.json` is `gpc:true` · `/termos` (LGPD), `/privacidade`, `/suporte` render · a **negative routing probe** confirms an unknown path still 404s |
